# borrow from https://github.com/rakhimovv/lvt/blob/master/vidgen/modeling/vq/vq_embedding.py
...
if __name__ == "__main__":
    n_codes = 1000
    code_dim = 128
    vqembedding = VQEmbedding(n_codes, code_dim, ema=True)
    num = 4
    dvqembedding = DVQEmbedding(num, n_codes, code_dim, ema=True)

    bs = 5
    c = code_dim
    h = w = 32
    z_e_x = torch.randn(bs, c, h, w)
    out = vqembedding(z_e_x, mode="st")
    print(out[0].shape, out[1].shape)

    out2 = dvqembedding(z_e_x, mode="st")
    print(out2[0].shape, out2[1].shape)
    print(out2[0][0, :, 0, 0], out2[1][0, :, 0, 0])
